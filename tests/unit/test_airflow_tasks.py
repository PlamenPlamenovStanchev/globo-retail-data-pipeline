...
from include.etl.transform_data.transformation_result import TransformationResult
...
class AirflowTaskTests(unittest.TestCase):
    ...
    def test_extract_task_returns_json_reference(self) -> None:
        ...
        with (
            ...
            result = retail_pipeline.extract_sales_task.function()
    ...
    def test_input_validation_stays_non_blocking_and_returns_reference(self) -> None:
        dataframe = pd.DataFrame({"qty": ["bad"]})
        validation_result = InputValidationResult(dataframe, False, 1, ["Column 'qty' failed dtype"])
        with (
            patch.object(retail_pipeline, "_read_intermediate_dataframe", return_value=dataframe),
            patch.object(retail_pipeline, "validate_sales_input", return_value=validation_result),
        ):
            result = retail_pipeline.validate_sales_input_task.function(REFERENCE)
    ...
    def test_sales_transform_persists_rejections_and_returns_reference(self) -> None:
        transform_result = TransformationResult(pd.DataFrame({"sales_id": [1]}), pd.DataFrame({"sales id": [2]}))
        transformed_reference = {**REFERENCE, "dataset": "sales", "key": "sales.parquet"}
        sales_write = S3WriteResult("test-bucket", "rejected/sales.parquet", 1, True)
        with (
            patch.object(retail_pipeline, "_read_intermediate_dataframe", return_value=pd.DataFrame()),
            patch.object(retail_pipeline, "transform_sales", return_value=transform_result),
            patch.object(retail_pipeline, "write_rejected_records", return_value=sales_write) as rejected_mock,
            patch.object(retail_pipeline, "_write_intermediate_dataframe", return_value=transformed_reference),
        ):
            result = retail_pipeline.transform_sales_task.function(REFERENCE)

        rejected_mock.assert_called_once_with(transform_result.rejected_rows, "sales")
        self.assertEqual(result, transformed_reference)
        self.assertNotIn("dataframe", result)
        json.dumps(result)
